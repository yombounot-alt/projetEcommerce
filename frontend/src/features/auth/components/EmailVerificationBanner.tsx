...
export function EmailVerificationBanner() {
  ...
  function handleClick() {
    requestVerification.mutate(undefined, {
      ...
      onError: (error) => {
        toast.error(error instanceof Error ? error.message : "Impossible d'envoyer l'email de vérification.");
      },
    });
  }
  ...
  return (
    <Alert variant="warning" className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <Button size="sm" variant="outline" onClick={handleClick} disabled={requestVerification.isPending}>
