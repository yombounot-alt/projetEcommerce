...
export interface OwnerNewOrderEmail {
  subject: string;
  html: string;
  text: string;
}

export function buildOwnerNewOrderEmail(order: IOrder): OwnerNewOrderEmail {
  const currency = order.currency;
  const orderDate = formatDate(order.createdAt);
  const adminUrl = `${env.FRONTEND_URL}/admin/orders/${order._id}`;
  const paymentMethodLabel = PAYMENT_METHOD_LABELS[order.payment.method] ?? order.payment.method;
  const paymentStatusLabel = PAYMENT_STATUS_LABELS[order.payment.status] ?? order.payment.status;
  const address = order.shippingAddress;

  const subject = `Nouvelle commande reçue — Commande n° ${order.orderNumber}`;
...
<html lang="fr">
          ...
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:8px;overflow:hidden;">
            <tr>
              <td style="background:#18181b;color:#ffffff;padding:20px 24px;">
                <h1 style="margin:0;font-size:18px;">Nouvelle commande reçue</h1>
              </td>
            </tr>
            <tr>
              <td style="padding:24px;">
                <p style="margin:0 0 16px;font-size:15px;">
                  Commande <strong>${escapeHtml(order.orderNumber)}</strong> passée le ${orderDate}.
                </p>

                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;margin-bottom:20px;">
                  <tr><td style="padding:4px 0;color:#71717a;width:160px;">Client</td><td style="padding:4px 0;">${escapeHtml(order.customerName)}</td></tr>
                  <tr><td style="padding:4px 0;color:#71717a;">Email</td><td style="padding:4px 0;">${escapeHtml(order.customerEmail)}</td></tr>
                  ${address.phone ? `<tr><td style="padding:4px 0;color:#71717a;">Téléphone</td><td style="padding:4px 0;">${escapeHtml(address.phone)}</td></tr>` : ""}
                  <tr><td style="padding:4px 0;color:#71717a;">Mode de paiement</td><td style="padding:4px 0;">${escapeHtml(paymentMethodLabel)}</td></tr>
                  <tr><td style="padding:4px 0;color:#71717a;">Statut du paiement</td><td style="padding:4px 0;">${escapeHtml(paymentStatusLabel)}</td></tr>
                </table>
                ...
                <a href="${adminUrl}" style="display:inline-block;background:#18181b;color:#ffffff;text-decoration:none;padding:10px 20px;border-radius:6px;font-size:14px;">Voir la commande</a>
              </td>
            </tr>
          </table>
        </td>
  ...
  const text = `Nouvelle commande reçue — Commande n° ${order.orderNumber}
...
Statut du paiement: ${paymentStatusLabel}
